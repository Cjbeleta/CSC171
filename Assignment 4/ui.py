...
class Gui:
    def __init__(self):
        ...
        pygame.init()
        pygame.display.set_caption('Spring Day Othello')

        bg = pygame.image.load('res/img/background.jpg')
        bg = pygame.transform.scale(bg, (720, 480))
        
        self.BLACK = (0, 0, 0)
        self.BACKGROUND = (100, 126, 135)
    ...
    def show_winner(self, player_color):
        ...
        win_white = pygame.image.load('res/img/winner-white.png')
        white_pos = win_white.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        win_black = pygame.image.load('res/img/winner-black.png')
        black_pos = win_black.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        tie = pygame.image.load('res/img/tie.png')
        tie_pos = tie.get_rect(centerx=self.screen.get_width() / 2, centery=280)

        
        if player_color == WHITE:
            msg = pygame.image.load('res/img/winner-white.png')
        elif player_color == BLACK:
            msg = pygame.image.load('res/img/winner-black.png')
        else:
            msg = pygame.image.load('res/img/tie.png')
        self.screen.blit(bg, [0,0])
        self.screen.blit(
            msg, msg.get_rect(
    ...
    def get_chosen_level(self):
        while True:
            ...
            title = pygame.image.load('res/img/level-select.png')
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=80)
            one_txt = pygame.image.load('res/img/level-one.png')
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=180)
            two_txt = pygame.image.load('res/img/level-two.png')
            two_pos = two_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=300)

            three_txt = pygame.image.load('res/img/level-three.png')
